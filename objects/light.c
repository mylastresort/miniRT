...
/*   Updated: 2023/01/18 17:11:27 by hjabbour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// static t_color	clamp1(t_color clr)
// {
// 	if (clr.r > 1.0)
// 		clr.r = 1.0;
// 	if (clr.r < 0.0)
...
t_color	light_coloring(const t_ray ray, t_sol sol, t_sp sp)
{
	...
	final_ambient = clr_multi_clr(sp.rgb, clr_multi_value(amb.la, amb.ka));
	...
	(void)ray;
	// tmp = clr_multi_clr(sp.rgb, diffuse_color);
	// tmp = clamp1(tmp);
	// return (clr_add_clr(final_ambient, tmp));
