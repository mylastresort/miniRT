...
/*   Updated: 2023/01/23 15:31:00 by stamim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sol	cy_get_intersections(const t_ray r, const t_cyl cyl)
{
	const float	a = powf(r.d.x, 2) + powf(r.d.y, 2) + powf(r.d.z, 2);
	const float	b = -2 * (r.d.x * (r.o.x + cyl.c.x) + r.d.y * (r.o.y + cyl.c.y)
			);
	const float	c = 2 * (r.o.x * cyl.c.x + r.o.y * cyl.c.y)
		+ powf(r.o.x, 2) + powf(cyl.c.x, 2) + powf(r.o.y, 2) + powf(cyl.c.y, 2)
		- powf(cyl.d, 2) / 4;
